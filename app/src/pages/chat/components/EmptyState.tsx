...
/** 推荐项（原型同款文案，图标改用 antd icons） */
const RECOMMENDATIONS: { icon: ReactNode; label: string; prompt: string }[] = [
  { icon: <BugOutlined />, label: '修复 approval 模块的中断 bug', prompt: '帮我修复 approval 模块在中断时 Promise 悬置的 bug' },
  { icon: <FieldTimeOutlined />, label: '给 Bash 工具加超时控制', prompt: '给 Bash 工具加上超时控制，默认 2 分钟' },
  { icon: <SearchOutlined />, label: '解释 agent loop 主循环', prompt: '解释 src/agent/loop.ts 的主循环逻辑' },
  { icon: <BoxPlotOutlined />, label: '初始化为 pnpm monorepo', prompt: '把当前项目初始化为 pnpm monorepo' },
];
